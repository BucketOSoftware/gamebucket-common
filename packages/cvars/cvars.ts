 ...
 */
export function consoleVariables<T extends object>(
    defaults: T,
    options: {
    ...
    const constantsHandler: ProxyHandler<{
        ...
        get(target, key) {
            ...
            if (key in target) {
                ...
                if (typeof protoValue === 'object') {
                    // Don't just return the object because we don't want the
                    // user to overwrite defaults!
        ...
        // Deleting a property effectively resets it to default (along with
        // everything beneath it if it's an object)
        ...
        saveCallbackHandle = requestIdleCallback(
            () => {
                ...
                const serialized =
                    ...
                    localStorage.setItem(localStorageKey, JSON.stringify(cvars))
            },
            { timeout: 1000 }
        )
    }
